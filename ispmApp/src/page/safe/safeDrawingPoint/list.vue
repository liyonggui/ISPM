...
<div class="safeDrawingPoint c_flex c_direction_v">
    <nav-bar left-arrow :left-text="$t('public.back')" @click-left="toBack" :title="$t('safe_drawing_point.routerTitle')">
        <div slot="right" class="van-nav-bar__text">
            <van-icon style="font-size: .2rem; margin-right: .05rem" name="add-o" @click="addHandler" v-permission="$permission.safe_drawing_point.add" />
            <van-icon style="font-size: .2rem;" name="more-o" @click="showMore=true" />
        </div>
    </nav-bar>
    <van-search v-model="searchFormModel.name" background="#f1f6f9" :placeholder="$t('safe_drawing_point.listSearchTip')" show-action @search="onSearch" v-permission="$permission.safe_drawing_point">
            ...
            <van-list v-model="loading" :finished="finished" @load="getData">
                <van-cell-group class="cellGroup">
                    <van-swipe-cell class="cardContainer" v-for="(item, index) in listData" :right-width="65" :item="item" :on-close="deleteItemHandler">
                        <van-cell class="cell" :key="index">
                                ...
                                <van-cell class="dangerPoint van-hairline--bottom">
                                    <van-cell>
                                        <van-row>
                                            <!--危险点名称-->
                                            <van-col>
                                                <span class="cellLabel">{{$t('safe_drawing_point.name')}} :</span>
                                                <span>{{item.name}}</span>
                                            </van-col>
                                        </van-row>
                                        <van-row>
                                            <!--危险点编号-->
                                            <span class="cellLabel">{{$t('safe_drawing_point.code')}} :</span>
                                            <span>{{item.code}}</span>
                                        </van-row>
                                        <van-row>
                                            <!--危险点状态-->
                                            <van-col>
                                                <span>{{$t('safe_drawing_point.status')}} :</span>
                                                <span>
                                        ...
                                        <van-row>
                                            <!--危险点分类-->
                                            <van-col>
                                                <span class="cellLabel">{{$t('safe_drawing_point.drawingPointType')}} :</span>
                                        ...
                                        <van-row>
                                            <!--危险点性质-->
                                            <van-col>
                                                <span class="cellLabel">{{$t('safe_drawing_point.pointProperty')}} :</span>
                            ...
                            <van-cell>
                                <van-row style="text-align: right;    margin-bottom: .05rem;">
                                    <!--删除-->
                                    <van-button size="small" :disabled="(item.dangerDelCol1+item.dangerDelCol2> 0)||(item.auditStatus=='audit_status_2'||item.auditStatus=='audit_status_3'||item.auditStatus=='audit_status_4')" type="danger" v-text="$t('public_button.delete')" v-permission="$permission.safe_drawing_point.delete" @click.stop="deleteClickHandler(item)"></van-button>
                                    <!--关闭-->
                                    <van-button size="small" :disabled="item.status!=='danger_status_3'" type="danger" v-text="$t('operation.close')" v-permission="$permission.safe_drawing_point.close" @click.stop="closeData(item)"></van-button>
                                    <!--审批-->
                                    <van-button size="small" v-if="item.audit&&item.auditStatus=='audit_status_2'" :disabled="item.auditStatus=='audit_status_3'" @click.stop="auditDetail(item)" v-permission="$permission.safe_drawing_point.consideration">
                                        {{$t('safe_drawing_point.audit')}}
                                    </van-button>
                                    <!--绑定二维码-->
                                    <van-button size="small" @click.stop="scanHandler(item)" v-permission="$permission.safe_drawing_point.edit">
                                        {{signType=='sign_type_1'? $t('safe_drawing_point.showQRCode'):$t('danger_project.showQRCode')}}
                                    </van-button>
                                    <!--提交审核-->
                                    <van-button size="small" v-if="item.auditStatus=='audit_status_1'" :disabled="(item.auditStatus=='audit_status_2'||item.auditStatus=='audit_status_3')" @click.stop="approveDetail(item)" v-permission="$permission.safe_drawing_point.approve">
                                        {{$t('safe_drawing_point.sumitAudit')}}
                                    </van-button>
                                    <!--发起整改-->
                                    <van-button size="small" v-if="(item.auditStatus=='audit_status_3')" @click.stop="renovate(item)" v-permission="$permission.safe_drawing_point.rectify">
                                        {{$t('safe_drawing_point.renovate')}}
                                    </van-button>
                                    <!--修改日志-->
                                    <van-button size="small" @click.stop="showLog(item)" v-permission="$permission.safe_drawing_point.view">
                        ...
                        <van-button :disabled="(item.dangerDelCol1+item.dangerDelCol2> 0)||(item.auditStatus=='audit_status_2'||item.auditStatus=='audit_status_3'||item.auditStatus=='audit_status_4')" type="danger" class="right_delete c_flex c_a_center c_j_center" slot="right" v-text="$t('public_button.delete')" v-permission="$permission.safe_drawing_point.delete"></van-button>
                    </van-swipe-cell>
                </van-cell-group>
            </van-list>
        </van-pull-refresh>
    ...
    <van-popup v-model="showMore">
        <van-button size="large" @click="sumitBatchAudit" v-permission="$permission.safe_drawing_point.approve">{{$t('safe_drawing_point.batchSumitAudit')}}</van-button>
        <van-button size="large" @click="onShowCompleteBatchAudit" v-permission="$permission.safe_drawing_point.consideration">{{$t('safe_drawing_point.batchCompleteAudit')}}</van-button>
        <van-button size="large" @click="scanSearchHandler" v-permission="$permission.safe_drawing_point.view">{{$t('safe_drawing_point.scanBarCode')}}</van-button>
    </van-popup>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        addHandler() {
            let model = 'create'
            this.$router.push('/safe/safeDrawingPoint/detail?model=' + model + '&rPath=plist')
        },
        ...
        itemClickHandler(item) {
            ...
            if (item.auditStatus == 'audit_status_5' || item.auditStatus == 'audit_status_2' || item.auditStatus == 'audit_status_3') {
                if (this.$permission.safe_drawing_point.view) {
                    model = 'view'
                    this.$router.push('/safe/safeDrawingPoint/detail?id=' + item.id + '&model=' + model + '&rPath=plist')
                }
            } else {
                if (this.$permission.safe_drawing_point.edit) {
                    model = 'edit'
                    this.$router.push('/safe/safeDrawingPoint/detail?id=' + item.id + '&model=' + model + '&rPath=plist')
                }
            }
        ...
        scanSearchSuccess(value) {
            if (value) {
                ...
                if (flag) {
                    let model = 'view'
                    this.$router.push('/safe/safeDrawingPoint/detail?id=' + this.rowData.id + '&model=' + model + '&rPath=plist')
                } else {
