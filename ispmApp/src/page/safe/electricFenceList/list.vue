...
<div class="safeDrawingPoint c_flex c_direction_v">
    <nav-bar left-arrow :left-text="$t('public.back')" @click-left="toBack" :title="$t('electric_fence_list.pointRouterTitle')">
        <div slot="right" class="van-nav-bar__text">
            <van-icon style="font-size: .2rem;margin-right: .05rem" name="add-o" @click="addHandler" v-permission="$permission.electric_fence_list.add" />
            <van-icon style="font-size: .2rem;" name="more-o" @click="showMore=true" />
        </div>
    </nav-bar>
    <van-search background="#f1f6f9" v-model="searchFormModel.name" :placeholder="$t('electric_fence_list.listSearchTip')" v-permission="$permission.electric_fence_list.view" show-action @search="onSearch">
                ...
                <van-cell-group class="cellGroup">
                    <van-swipe-cell class="cardContainer" v-for="(item, index) in listData" :right-width="65" :item="item" :on-close="deleteItemHandler">
                        <van-cell class="cell" :key="index">
                            <div @click="itemClickHandler(item)">
                                <div class="cellContainer van-hairline--bottom">
                                    ...
                                    <van-row type="flex" class="region" justify="space-between">
                                        <van-col span="20">
                                            <span>{{$t('safe_drawing_point.region')}} :</span>
                                            <!--区域/楼宇/分部分项/楼层-->
                                ...
                                <van-cell style="background: none;" class="dangerPoint van-hairline--bottom">
                                    <van-cell>
                                        <van-row>
                                            <!--危险点名称-->
                                            <van-col>
                                        ...
                                        <van-row>
                                            <!--危险点状态-->
                                            <van-col>
                                                <span class="cellLabel">{{$t('electric_fence_list.status')}} :</span>
                                                <span>
                                            <van-tag  v-if="item.status" plain :type="calcStatusType(item.status)">
                                        ...
                                        <van-row>
                                            <!--危险点性质-->
                                            <van-col>
                                                <span class="cellLabel">{{$t('electric_fence_list.pointProperty')}} :</span>
                                                <span>{{$translate({key: 'point_property',value:item.pointProperty})}}</span>
                                            </van-col>
                                        </van-row>
                                        <van-row>
                                            <!--巡查频次-->
                            ...
                            <van-row style="text-align:right;margin-bottom:.05rem;margin-top:.05rem;margin-right:.05rem;">
                                <!--删除-->
                                <van-button size="small" :disabled="(item.dangerDelCol1+item.dangerDelCol2> 0)||(item.auditStatus=='audit_status_2'||item.auditStatus=='audit_status_3'||item.auditStatus=='audit_status_4')" type="danger" v-text="$t('public_button.delete')" v-permission="$permission.electric_fence_list.delete" @click.stop="deleteClickHandler(item)"></van-button>
                                <!--关闭-->
                                <van-button size="small" :disabled="item.status!=='rail_status_1'" type="danger" v-text="$t('operation.close')" v-permission="$permission.electric_fence_list.close" @click.stop="closeData(item)"></van-button>
                                <!--审批-->
                                <van-button size="small" v-if="item.audit&&item.auditStatus=='audit_status_2'" :disabled="item.auditStatus=='audit_status_3'" @click.stop="auditDetail(item)" v-permission="$permission.electric_fence_list.consideration">
                                    {{$t('safe_drawing_point.audit')}}
                                </van-button>
                                <!--绑定二维码-->
                                <van-button size="small" v-if="item.auditStatus=='audit_status_1'" @click.stop="scanHandler(item)" v-permission="$permission.electric_fence_list.edit">
                                    {{$t('safe_drawing_point.showQRCode')}}
                                </van-button>
                                <!--提交审核-->
                                <van-button size="small" v-if="item.auditStatus=='audit_status_1'" :disabled="(item.auditStatus=='audit_status_2'||item.auditStatus=='audit_status_3')" @click.stop="approveDetail(item)" v-permission="$permission.electric_fence_list.approve">
                                    {{$t('safe_drawing_point.sumitAudit')}}
                                </van-button>
                                <!--修改日志-->
                                <van-button size="small" @click.stop="showLog(item)" v-permission="$permission.electric_fence_list.view">
                                    {{$t('safe_drawing_point.modifyLog')}}
                                </van-button>
                            </van-row>
                        </van-cell>
                        <van-button :disabled="(item.dangerDelCol1+item.dangerDelCol2> 0)||(item.auditStatus=='audit_status_2'||item.auditStatus=='audit_status_3'||item.auditStatus=='audit_status_4')" type="danger" class="right_delete c_flex c_a_center c_j_center" slot="right" v-text="$t('public_button.delete')" v-permission="$permission.electric_fence_list.delete"></van-button>
                    </van-swipe-cell>
                </van-cell-group>
            </van-list>
        </van-pull-refresh>
    ...
    <van-popup v-model="showMore">
        <van-button size="large" @click="sumitBatchAudit" v-permission="$permission.electric_fence_list.approve">{{$t('safe_drawing_point.batchSumitAudit')}}</van-button>
        <van-button size="large" @click="onShowCompleteBatchAudit" v-permission="$permission.electric_fence_list.consideration">{{$t('safe_drawing_point.batchCompleteAudit')}}</van-button>
        <van-button size="large" @click="scanSearchHandler" v-permission="$permission.electric_fence_list.view">{{$t('safe_drawing_point.scanBarCode')}}</van-button>
    </van-popup>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            filterInfo.type = this.pointType
            this.$axios
        ...
        addHandler() {
            let model = 'create'
            this.$router.push('/safe/electricFenceList/detail?model=' + model + '&rPath=plist')
        },
        ...
        closeData(rowData) {
            this.$dialog.confirm({
                ...
                let submitModel = cloneDeep(rowData)
                    ...
                    submitModel.status = 'rail_status_2' // 关闭
        ...
        itemClickHandler(item) {
            ...
            let model
            if (item.auditStatus == 'audit_status_2' || item.auditStatus == 'audit_status_3') {
                if (this.$checkPermission(this.$permission.electric_fence_list.view)) {
                    model = 'view'
                    this.$router.push('/safe/electricFenceList/detail?id=' + item.id + '&model=' + model + '&rPath=plist')
                }

            } else {

                if (this.$checkPermission(this.$permission.electric_fence_list.edit)) {
                    model = 'edit'
                    this.$router.push('/safe/electricFenceList/detail?id=' + item.id + '&model=' + model + '&rPath=plist')
                }
        ...
        auditDetail(item) {
            if (item.type == "point_type_1") {
                this.$router.push('/safe/electricFenceList/detail?id=' + item.id + '&taskId=' + item.taskId + '&rPath=plist')
            } else {
                this.$router.push('/safe/projectElectricFenceList/detail?id=' + item.id + '&taskId=' + item.taskId + '&rPath=plist')
            }
        },
        //显示批量审批界面
        ...
        scanSearchSuccess(value) {
            if (value) {
                ...
                if (flag) {
                    let model = 'view'
                    this.$router.push('/safe/electricFenceList/detail?id=' + this.rowData.id + '&model=' + model + '&rPath=plist')
                } else {
    ...
    mounted() {
        this.addEventBackButtonBack();
        this.pointType = 'point_type_1'
        this.signType = "sign_type_2"
    }
};
</script>
