<template>
<tms-page>
    <nav-bar slot="header" left-arrow :left-text="$t('public.back')" @click-left="toBack" :title="$t('electric_fence_list.electricFenceDetail') ">
        <div slot="right" v-if="formModel.auditStatus=='audit_status_4' || formModel.auditStatus=='audit_status_1'" class="van-nav-bar__text">
            <van-icon style="font-size: .2rem;margin-right: .05rem" name="edit" @click="isEdit=!isEdit" v-permission="$permission.electric_fence_list.edit" />
        </div>
    </nav-bar>
    <div slot="content">
        <van-cell-group>
            ...
            <!--危险点名称-->
            <van-field :maxlength="100" :disabled="!isEdit" @blur="formModel.name=formModel.name.trim()" required input-align="right" :label=" $t('electric_fence_list.railName')" :placeholder="isEdit?$t('electric_fence_list.railNameHit'):''" v-model="formModel.name" />
            <!--危险点编号-->
            <van-cell :title="$t('electric_fence_list.railCode')" :value="formModel.code" />
            <!--危险点二维码-->
            <van-field :maxlength="100" @blur="formModel.qrCode=formModel.qrCode.trim()" v-model="formModel.qrCode" input-align="right" :disabled="!isEdit" :label="$t('safe_drawing_point.showQRCode')" :placeholder="isEdit?$t('electric_fence_list.barCodeHit'):''">
...
export default {
    ...
    methods: {
        toBack() {
            if (this.$route.query.rPath == 'plist') this.$router.push('/safe/electricFenceList/list')
            else this.$router.push('/task/todo/list')

        },
        //获取图纸信息
        getDrawing() {
            if (!!this.formModel.drawing) {
                let params = {
