<template>
<tms-page>
    <nav-bar slot="header" left-arrow :left-text="$t('public.back')" @click-left="toBack" :title="$t('danger_project.pointDetail')">
        <div slot="right" v-if="formModel.auditStatus=='audit_status_4' || formModel.auditStatus=='audit_status_1'" class="van-nav-bar__text">
            <van-icon style="font-size: .2rem;margin-right: .05rem" name="edit" @click="isEdit=!isEdit" v-permission="$permission.danger_project_list.edit" />
        </div>
    </nav-bar>
    <div slot="content">
        <van-cell-group>
            ...
            <!--危险点名称-->
            <van-field @blur="formModel.name=formModel.name.trim()" :maxlength="100" required :disabled="!isEdit" input-align="right" :label="$t('danger_project.pointName')" :placeholder="isEdit?$t('safe_drawing_point.nameHit'):''" v-model="formModel.name" />
            <!--危险点编号-->
            <van-cell :title="$t('danger_project.pointCode')" :value="formModel.code" />
            <!--危险点二维码-->
            <van-field  @blur="formModel.qrCode=formModel.qrCode.trim()"  :maxlength="100" v-model="formModel.qrCode" input-align="right" :disabled="!isEdit" :label="$t('safe_drawing_point.showQRCode')" :placeholder="isEdit?$t('safe_drawing_point.barCodeHit'):''">
                <van-icon v-if="isEdit" style="font-size: 20px;" slot="button" name="qr" @click="scanHandler" />
            </van-field>
            <!--危险点分类-->
            <van-cell required :title="$t('danger_project.drawingPointType')" :value="$translate({key: 'drawing_point_type', value: formModel.drawingPointType})" :is-link="isEdit" @click="onShowPointTypePopup" />
            ...
            <!--危险点性质-->
            <van-cell required :title="$t('danger_project.pointProperty')" :value="$translate({key: 'point_property', value: formModel.pointProperty})" :is-link="isEdit" @click="onShowPointPropertyPopup" />
            ...
            <!--危险点状态-->
            <van-cell required :title="$t('danger_project.status')" :value="$translate({key: 'danger_status', value: formModel.status})" :is-link="isEdit" @click="onShowPointStatusPopup" />
            ...
            <!--危险点描述-->
            <van-field :maxlength="1000" @blur="formModel.remarks=formModel.remarks.trim()"  :disabled="!isEdit" type="textarea" autosize input-align="right" :label="$t('safe_drawing_point.remarks')" :placeholder="isEdit?$t('safe_drawing_point.remarksHit'):''" v-model="formModel.remarks" />
            <!--审核意见-->
            <van-field :maxlength="1000"  @blur="formModel.comment=formModel.comment.trim()" type="textarea" v-if="viewModel=='submitAudit'" autosize input-align="right" :label="$t('safe_drawing_point.auditRemarks')" :placeholder="$t('safe_drawing_point.auditRemarks')" v-model="formModel.comment" />
        </van-cell-group>
        <!--审批历史-->
...
export default {
    ...
    methods: {
        toBack() {
            if (this.$route.query.rPath == 'plist') this.$router.push('/danger_project_list')
        ...
        addEventBackButtonBack() {
            if (sessionStorage.plusready === "done") {
                ...
                plus.key.addEventListener("backbutton", () => {
                    console.log("addEventListener_Safe_Detail")
                    this.toBack();
                });
            }
        },
    ...
    mounted() {
        ...
        this.formModel.signType = "sign_type_3"
