<template>
<div class="vdr" @touchmove="handleMove" @mousedown.stop="elmDown" @touchstart.stop="elmDown" :style="style" :class="{ draggable: draggable, resizable: resizable, active: enabled, dragging: dragging, resizing: resizing }">
    <div class="handle" v-if="resizable" v-for="handle in handles" :class="'handle-' + handle" :style="{ display: enabled ? 'block' : 'none', transform: `scale(${1/zoom})`}" @mousedown.stop.prevent="handleDown(handle, $event)" @touchstart.stop.prevent="handleDown(handle, $event)"></div>
    <slot></slot>
</div>
</template>
...
export default {
    ...
    mounted() {
        ...
        document.documentElement.addEventListener('touchmove', this.handleMove, {
            passive: false
        })
    ...
    beforeDestroy() {
        ...
        document.documentElement.removeEventListener('touchmove', this.handleMove, {
            passive: false
        })
        document.documentElement.removeEventListener('touchend', this.deselect, true)
    ...
    methods: {
        reviewDimensions() {
            ...
            if (this.parent) {
                ...
                if (this.x > this.parentW) this.width = parentW - this.x

                if (this.y > this.parentH) this.height = parentH - this.y
